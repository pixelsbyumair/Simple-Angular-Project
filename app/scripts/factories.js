angular.module('quizApp').factory('quizFactory', function(){
	...
	quizFac.activateQuiz = false;
	quizFac.quizNumber = 1;
	quizFac.questions = quizQuestions;
	quizFac.correctAnswers = correctAnswers;

	/*quizFac.toggleQuiz = function(state) {
		quizFac.activateQuiz = state;	
	}*/

	quizFac.getTurtles = function() {
		return turtlesData;
	}

	quizFac.getQuestions = function() {
		return quizQuestions;
	}

	/*quizFac.increment = function() {
		if (quizQuestions[quizFac.quizNumber-1].selected==null) {
			quizQuestions[quizFac.quizNumber-1].selected = false;
		}
		quizFac.quizNumber++;

		if (quizFac.quizNumber>quizQuestions.length) {
			alert("Quiz Ended!");
		}
	}*/

	/*quizFac.selectQuestion = function(){
		quizQuestions[quizFac.quizNumber-1].selected = true;
	}*/
...
}).factory('metricsFactory', ['quizFactory', function(quizFactory){
	...
	quizMetrics.quizActive = false;
	quizMetrics.resultsActive = false;
	quizMetrics.quizFactory = quizFactory;
	quizMetrics.correct = 0;


	quizMetrics.markQuiz = function() {
		for (var i=0; i<quizMetrics.quizFactory.questions.length; i++) {
			
			if (quizMetrics.quizFactory.questions[i].selected == quizMetrics.quizFactory.correctAnswers[i]) {
				quizMetrics.quizFactory.questions[i].correct = true;
				quizMetrics.correct++;
			} else {
				quizMetrics.quizFactory.questions[i].correct = false;
			}
	...
	quizMetrics.changeState = function(metric, state) {
		
		if (metric=="quiz") {
			quizMetrics.quizActive = state;

		} else if (metric=="results") {
			quizMetrics.resultsActive = state;
